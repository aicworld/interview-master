...
async def on_message(message: cl.Message):
    if(message.type == "init_message"):
        ...
        await score_msg.set_score(score)
...
def determine_prompt(id,difficulty_value):
    ...
    if id == "1":
        return f'''你是一个高级面试机器人，专为评估潜在的GoLang工程师的技术能力、编程经验以及对待工作的态度而设计。
...
def determine_grade_prompt(id,difficulty_value):
    ...
    if id == "1":
        return f'''
            ...
            在进行评分时，请遵循以下标准：
            - 技术能力（0到{difficulty_v1}分）：考察候选人对Golang语言的掌握程度，包括语法、并发处理、内存管理等方面。候选人的答案如果显示出对基础概念的误解，可能会得到0分。
            - 项目经验（0到{difficulty_v2}分）：评估候选人过往参与的项目，特别是在Golang相关项目中的角色、贡献和解决问题的能力。如果候选人无法提供具体的经验或项目细节，或者示例不相关，可能会得到0分。
            - 沟通能力和问题解决能力（0到{difficulty_v3}分）：通过候选人对问题的回答，评价其逻辑思维、沟通表达和问题解决的能力。如果候选人在沟通上存在明显问题，如回避问题或答非所问，可能会得到0分。
            - 对职位的兴趣和热情（0到{difficulty_v4}分）：了解候选人对这个Golang工程师职位的兴趣程度以及他们对未来工作的热情和期待。缺乏热情或兴趣的表现可能会导致0分。
