...
client = AsyncOpenAI(
    api_key="sk-kzh5j9p79xC9404fnLTrBfsbUuEGoTVYCEnHwy73QktGqDDn",
...
async def on_message(message: cl.Message):
    if(message.type == "init_message"):
        ...
        score,result = extract_last_bracket_number_and_preceding_text(grade.content)
        score_msg = cl.Message(content="")
        await score_msg.send_with_score(score)
...
def determine_grade_prompt(id,difficulty_value):
    ...
    if id == "1":
        return f'''你是一个高级面试机器人，专为评估潜在的GoLang工程师的技术能力、编程经验以及对待工作的态度而设计。
            ...
            请在完成一系列问答后，根据上述评分标准，综合候选人的回答内容，仅仅给出一个总体评分，以 [分数] 的形式放到答复的末尾。例如，如果总分为5分，则在回答结束后添加 [5]。同时，请提出下一个问题，以持续评估候选人的能力。'''
    else:
        return "Default prompt"